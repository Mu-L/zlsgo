...
func (b *Balancer[T]) selectNode(strategy BalancerStrategy) (*balancerNode[T], error) {
...
func (b *Balancer[T]) Run(fn func(node T) (normal bool, err error), maxRetry int, strategy ...BalancerStrategy) error {
	if fn == nil {
	...
	err := zutil.DoRetry(maxRetry, func() error {
		node, err := b.selectNode(s)
		if err != nil {
			return err
		}
